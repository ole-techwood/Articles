...
- Identify existing patterns and conventions from surrounding source
- Detect the package manager from the repository's package metadata and lockfiles (for example, `packageManager` in `package.json`, `pnpm-lock.yaml`, `yarn.lock`, `package-lock.json`, or `bun.lockb`/`bun.lock`), and use that manager consistently in all verification commands
...
- [ ] Tests pass: `[detected-package-manager] run test`
- [ ] Build succeeds: `[detected-package-manager] run build`
