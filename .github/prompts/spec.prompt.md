...
Follow the caveman communication skill exactly as defined in [SKILL.md](../../.agents/skills/caveman/). All responses must be terse, drop all filler, use fragments. This mode is active for **every response in this session**. The final spec document itself must NOT use caveman mode — use clear, standard technical writing for the document only.
...
3. **Architecture & Design** — Tech stack, key components, data flow. _You can provide additional instructions if you have them._
4. **Commands & Operations** — How to build, test, run, deploy
5. **Code Style & Conventions** — Naming, structure, testing expectations. _You can provide additional instructions if you have them._
